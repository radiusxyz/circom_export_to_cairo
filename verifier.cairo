...
func verifyingKey{range_check_ptr : felt}() -> (vk : VerifyingKey):
	...
	let alfa1 : G1Point = BuildG1Point(
        0x4128632272367203679867622, 0x59316817566498460416086069, 0x3365667018767545775131225,
        0x69628749977975432876719432, 0x212134230527877088973743, 0x1075080431125946600950867
    )

    let beta2 : G2Point = BuildG2Point(
        0x52011725285810035473763165, 0x54295241303750663612856864, 0x855464291522136583602551,
        0x70081545844137829002204336, 0x2511906413928923024452533, 0x1691997074693486054649924,
        0x4537538041209377186537467, 0x24947520614210272376506580, 0x3071346993298696983506611,
        0x17994271894161191434014939, 0x49856466110497178836537250, 0x2887063480940427602724375
    )

    let gamma2 : G2Point = BuildG2Point(
        0x50657168248156029357068994, 0x75996009454876762764004566, 0x1931027739743020521039371,
        0x37301332318871981678327533, 0x1933688095072267321168861, 0x1813645754675075253282464,
        0x62330281573273320105875291, 0x46796958261627355650117058, 0x681950549513863929186793,
        0x4625471277591146859756970, 0x7264017464802134638568497, 0x1419180249680038923119287
    )
    let delta2 : G2Point = BuildG2Point(
        0x59029551946905996687139647, 0x10635795411383158388757996, 0x3394954252780198474753723,
        0x55556321251482204824499066, 0x14411815194499862502303927, 0x991678603497082678317083,
        0x10492808113093199653976063, 0x40319643329156450904203002, 0x1455914935118052809315061,
        0x19453565443744424783203405, 0x73535876413782221574839063, 0x2785923207387608827358665
    )
        
    let (IC : G1Point*) = alloc()
    
    let point_0 : G1Point =  BuildG1Point( 
        0x50356480864665616340790615, 0x66413467357508129376590561, 0x240810614190059188556337,
        0x39375195015652531028999878, 0x49295805580131804920830610, 0x2366913347462842214243150)
    assert IC[0] =  point_0                                   
    
    let point_1 : G1Point =  BuildG1Point( 
        0x77148511171766230904552821, 0x59066045416753182757480107, 0x683460483585951091873934,
        0x56114984742584415013512256, 0x13261991128747846248358577, 0x1028073059353397348829225)
    assert IC[1] =  point_1                                   
...
func main{output_ptr: felt*, pedersen_ptr : HashBuiltin*, range_check_ptr: felt}():
    ...
    assert[a_ptr + 3] = 0x21845360519278039434162382
    assert[a_ptr + 4] = 0x37032914906954177889717841
    assert[a_ptr + 5] = 0x422447693251006666742151

    let (b1_ptr) = alloc()
    assert[b1_ptr] = 0x39203446000482540257637925
    assert[b1_ptr + 1] = 0x8372331185708158742649818
    assert[b1_ptr + 2] = 0x1940944301260288891776770
    assert[b1_ptr + 3] = 0x14539866263615908413985012
    assert[b1_ptr + 4] = 0x28351972057156804035626776
    assert[b1_ptr + 5] = 0x3102459511026671344229042

    let (b2_ptr) = alloc()
    assert[b2_ptr] = 0x61583281797485710770918457
    assert[b2_ptr + 1] = 0x6382783199640232524620208
    assert[b2_ptr + 2] = 0x1641855124893934125318615
    assert[b2_ptr + 3] = 0x69691338484345404523556820
    assert[b2_ptr + 4] = 0x22583992049999120098074020
    assert[b2_ptr + 5] = 0x18101688457955382914455237

    let (c_ptr) = alloc()
    assert[c_ptr] = 0x37449182766458286666487412
    assert[c_ptr + 1] = 0x67949700759915101588395634
    assert[c_ptr + 2] = 0x1615312652241491981991622
    assert[c_ptr + 3] = 0x18053195823741242528076033
    assert[c_ptr + 4] = 0x63793431666194470068739038
    assert[c_ptr + 5] = 0x2763392202904072211382205
